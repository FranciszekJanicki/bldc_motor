...
static bldc_motor_err_t bldc_motor_hall_initialize(bldc_motor_t const* bldc_motor)
{
    assert(bldc_motor);

    if (bldc_motor->interface.hall_initialize) {
        return bldc_motor->interface.hall_initialize(bldc_motor->interface.hall_user);
    }

    return BLDC_MOTOR_ERR_NULL;
}

static bldc_motor_err_t bldc_motor_hall_deinitialize(bldc_motor_t const* bldc_motor)
{
    assert(bldc_motor);

    if (bldc_motor->interface.hall_deinitialize) {
        return bldc_motor->interface.hall_deinitialize(bldc_motor->interface.hall_user);
    }
...
static bldc_motor_err_t bldc_motor_pwm_initialize(bldc_motor_t const* bldc_motor,
                                                  bldc_motor_channel_t channel)
{
    assert(bldc_motor);

    if (bldc_motor->interface.pwm_initialize) {
        return bldc_motor->interface.pwm_initialize(bldc_motor->interface.pwm_user, channel);
    }

    return BLDC_MOTOR_ERR_NULL;
}

static bldc_motor_err_t bldc_motor_pwm_deinitialize(bldc_motor_t const* bldc_motor,
                                                    bldc_motor_channel_t channel)
{
    assert(bldc_motor);

    if (bldc_motor->interface.pwm_deinitialize) {
        return bldc_motor->interface.pwm_deinitialize(bldc_motor->interface.pwm_user, channel);
    }
