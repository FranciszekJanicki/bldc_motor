...
static bldc_motor_err_t
bldc_motor_hall_initialize(bldc_motor_t const *bldc_motor) {
  assert(bldc_motor);

  if (bldc_motor->interface.hall_initialize) {
    return bldc_motor->interface.hall_initialize(
        bldc_motor->interface.hall_user);
  }
...
static bldc_motor_err_t
bldc_motor_hall_deinitialize(bldc_motor_t const *bldc_motor) {
  assert(bldc_motor);

  if (bldc_motor->interface.hall_deinitialize) {
    return bldc_motor->interface.hall_deinitialize(
        bldc_motor->interface.hall_user);
  }
...
static bldc_motor_err_t
bldc_motor_hall_get_position(bldc_motor_t const *bldc_motor,
                             bldc_motor_position_t *position) {
  assert(bldc_motor);

  if (bldc_motor->interface.hall_get_position) {
    return bldc_motor->interface.hall_get_position(
        bldc_motor->interface.hall_user, position);
  }
...
static bldc_motor_err_t
bldc_motor_pwm_initialize(bldc_motor_t const *bldc_motor,
                          bldc_motor_channel_t channel) {
  assert(bldc_motor);

  if (bldc_motor->interface.pwm_initialize) {
    return bldc_motor->interface.pwm_initialize(bldc_motor->interface.pwm_user,
                                                channel);
  }
...
static bldc_motor_err_t
bldc_motor_pwm_deinitialize(bldc_motor_t const *bldc_motor,
                            bldc_motor_channel_t channel) {
  assert(bldc_motor);

  if (bldc_motor->interface.pwm_deinitialize) {
    return bldc_motor->interface.pwm_deinitialize(
        bldc_motor->interface.pwm_user, channel);
  }
...
static bldc_motor_err_t bldc_motor_pwm_start(bldc_motor_t const *bldc_motor,
  ...
  if (bldc_motor->interface.pwm_start) {
    return bldc_motor->interface.pwm_start(bldc_motor->interface.pwm_user,
                                           channel);
  }
...
static bldc_motor_err_t bldc_motor_pwm_stop(bldc_motor_t const *bldc_motor,
  ...
  if (bldc_motor->interface.pwm_stop) {
    return bldc_motor->interface.pwm_stop(bldc_motor->interface.pwm_user,
                                          channel);
  }
...
static bldc_motor_err_t
bldc_motor_pwm_stop_all(bldc_motor_t const *bldc_motor) {
  assert(bldc_motor);
...
static bldc_motor_err_t
bldc_motor_pwm_set_compare(bldc_motor_t const *bldc_motor,
                           bldc_motor_channel_t channel, uint32_t compare) {
  assert(bldc_motor);
...
static bldc_motor_err_t
bldc_motor_commutation_forward_0(bldc_motor_t const *bldc_motor,
                                 uint32_t compare) {
  assert(bldc_motor);

  bldc_motor_err_t err =
      bldc_motor_pwm_start(bldc_motor, BLDC_MOTOR_CHANNEL_1H);
...
static bldc_motor_err_t
bldc_motor_commutation_forward_60(bldc_motor_t const *bldc_motor,
                                  uint32_t compare) {
  assert(bldc_motor);

  bldc_motor_err_t err =
      bldc_motor_pwm_start(bldc_motor, BLDC_MOTOR_CHANNEL_2H);
...
static bldc_motor_err_t
bldc_motor_commutation_forward_120(bldc_motor_t const *bldc_motor,
                                   uint32_t compare) {
  assert(bldc_motor);

  bldc_motor_err_t err =
      bldc_motor_pwm_start(bldc_motor, BLDC_MOTOR_CHANNEL_3H);
  err |= bldc_motor_pwm_start(bldc_motor, BLDC_MOTOR_CHANNEL_1L);
...
static bldc_motor_err_t
bldc_motor_commutation_forward_180(bldc_motor_t const *bldc_motor,
                                   uint32_t compare) {
  assert(bldc_motor);
...
static bldc_motor_err_t
bldc_motor_commutation_forward_240(bldc_motor_t const *bldc_motor,
                                   uint32_t compare) {
  assert(bldc_motor);
...
static bldc_motor_err_t
bldc_motor_commutation_forward_300(bldc_motor_t const *bldc_motor,
                                   uint32_t compare) {
  assert(bldc_motor);
...
static bldc_motor_err_t
bldc_motor_commutation_backward_0(bldc_motor_t const *bldc_motor,
                                  uint32_t compare) {
  assert(bldc_motor);
...
static bldc_motor_err_t
bldc_motor_commutation_backward_60(bldc_motor_t const *bldc_motor,
                                   uint32_t compare) {
  assert(bldc_motor);
...
static bldc_motor_err_t
bldc_motor_commutation_backward_120(bldc_motor_t const *bldc_motor,
                                    uint32_t compare) {
  assert(bldc_motor);
...
bldc_motor_err_t
bldc_motor_commutation_backward_180(bldc_motor_t const *bldc_motor,
                                    uint32_t compare) {
  assert(bldc_motor);
...
bldc_motor_err_t
bldc_motor_commutation_backward_240(bldc_motor_t const *bldc_motor,
                                    uint32_t compare) {
  assert(bldc_motor);
...
static bldc_motor_err_t
bldc_motor_commutation_backward_300(bldc_motor_t const *bldc_motor,
                                    uint32_t compare) {
  assert(bldc_motor);
...
bldc_motor_err_t
bldc_motor_initialize(bldc_motor_t *bldc_motor,
  ...
  bldc_motor_err_t err = bldc_motor_hall_initialize(bldc_motor);
  for (bldc_motor_channel_t channel = 0; channel < 6; ++channel) {
    err |= bldc_motor_pwm_initialize(bldc_motor, channel);
  }
...
bldc_motor_err_t bldc_motor_deinitialize(bldc_motor_t *bldc_motor) {
  ...
  bldc_motor_err_t err = bldc_motor_hall_deinitialize(bldc_motor);
  for (bldc_motor_channel_t channel = 0; channel < 6; ++channel) {
    err |= bldc_motor_pwm_deinitialize(bldc_motor, channel);
  }
...
static bldc_motor_err_t (*forward_commutations[])(bldc_motor_t const *,
                                                  uint32_t) = {
...
bldc_motor_err_t bldc_motor_commutation_forward(bldc_motor_t *bldc_motor,
  ...
  if (position != bldc_motor->position && position > 0b000 &&
      position < 0b111) {
...
static bldc_motor_err_t (*backward_commutations[])(bldc_motor_t const *,
                                                   uint32_t) = {
